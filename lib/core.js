...
function loadStyleNotes(accountId = null) {
  const raw = readStyleNotesRaw(accountId);
  return raw
    .split(/\r?\n/)
...
function readStyleNotesRaw(accountId = null) {
  const { styleNotesPath } = require("./accounts").resolvePaths(accountId);
  if (!fs.existsSync(styleNotesPath)) return DEFAULT_STYLE_NOTES;
  return fs.readFileSync(styleNotesPath, "utf8");
}

function writeStyleNotesRaw(text, accountId = null) {
  const { styleNotesPath } = require("./accounts").resolvePaths(accountId);
  fs.writeFileSync(styleNotesPath, text, "utf8");
}
...
function loadExamples(accountId = null) {
  ...
  const personal = require("./accounts").loadPersonalExamplesText(accountId);
  return [shared, personal].filter(Boolean).join("\n\n---\n\n");
}

function loadBannedWords(accountId = null) {
  return readBannedWordsRaw(accountId)
    .split(/\r?\n/)
    .map((line) => line.trim())
...
function readBannedWordsRaw(accountId = null) {
  const { bannedWordsPath } = require("./accounts").resolvePaths(accountId);
  if (!fs.existsSync(bannedWordsPath)) return "";
  return fs.readFileSync(bannedWordsPath, "utf8");
}

function writeBannedWordsRaw(text, accountId = null) {
  const { bannedWordsPath } = require("./accounts").resolvePaths(accountId);
  fs.writeFileSync(bannedWordsPath, text, "utf8");
}

function buildSystemPrompt(styleGuide, examples, bannedWords, styleNotes) {
...
# 出力ルール
- 上記スタイルガイドの6見出し構成（【登録動機】【交際タイプ】【ルックス】【性格】【金銭感覚】【最後に】）をこの順番で必ず使用すること。
...
# 追加出力：おすすめの男性像
6見出しの倶楽部コメントを出力し終えたら、改行を2つ挟んで ${RECOMMEND_MARKER} という行だけを出力し、
...
async function generateIntro({
  ...
  const examples = loadExamples(accountId);
  const bannedWords = loadBannedWords(accountId);
  const styleNotes = loadStyleNotes(accountId);

  const details = { height, weight, bust, type, age, occupation, impression };
  const referenceInfo = buildReferenceInfo(memo, name, details);
  const systemPrompt = buildSystemPrompt(styleGuide, examples, bannedWords, styleNotes);
  ...
  const missingHeadings = checkHeadings(output);
  const closingRepetition = checkClosingRepetition(output);
  const missingRecommendation = !recommendation;

  const consistencyWarnings = await checkConsistency(client, usedModel, referenceInfo, output);
...
function saveHistory({
  ...
  const { historyDir } = require("./accounts").resolvePaths(accountId);
  ...
  fs.writeFileSync(path.join(historyDir, `${id}.json`), JSON.stringify(record, null, 2), "utf8");
  return record;
}
