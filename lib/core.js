...
function getRequiredHeadings(includeConcerns = false) {
  if (!includeConcerns) return REQUIRED_HEADINGS;
  const idx = REQUIRED_HEADINGS.indexOf("【最後に】");
  return [...REQUIRED_HEADINGS.slice(0, idx), CONCERNS_HEADING, ...REQUIRED_HEADINGS.slice(idx)];
}
...
function buildSystemPrompt(styleGuide, examples, bannedWords, styleNotes, includeConcerns = false) {
  ...
  const styleNotesBlock = styleNotes && styleNotes.trim()
    ...
    : "";
  const headings = getRequiredHeadings(includeConcerns);
  ...
  const concernsInstruction = includeConcerns
    ...
    : `\n- 今回は【懸念点】を含める指定がされていない。スタイルガイド内に【懸念点】の説明があっても、面接メモの内容に気になる点が読み取れたとしても、【懸念点】の見出しは絶対に追加しないこと（指定が無い限り常に6見出し構成のみ）。`;
...
- メモに書かれていない事実を創作しないこと（誇張は避け、印象表現の範囲に留める）。${concernsInstruction}
...
function checkHeadings(text, includeConcerns = false) {
  return getRequiredHeadings(includeConcerns).filter((h) => !text.includes(h));
}
...
async function generateIntro({
  ...
  const systemPrompt = buildSystemPrompt(styleGuide, examples, bannedWords, styleNotes, includeConcerns);
  ...
  const missingHeadings = checkHeadings(output, includeConcerns);
